...
<html>
	...
	<div id="container">
		...
		<div id="maincontent">
			...
			<div>
				<s:form theme="simple" action="kaoQinSearch" >
				<table width="100%" style="font-size:14px;color:#404040;">
				<tr>
					<td>
					请选择部门:
					<s:select name="departId" label="" labelposition="left" 
					list="departList" listKey="key" listValue="value"/>
					</td>
					<td>
					状态:
					<s:select name="attendType" label="请选择状态：" labelposition="left" 
					list="stateList" listKey="key" listValue="value"/>
					</td>
					<td>
					起始时间:<s:datetimepicker name="startTime" toggleType="explode" value="today"/>
					</td>
					<td>
					终止时间:<s:datetimepicker name="endTime" toggleType="explode" value="today"/>
					</td>
					<td>
						<s:submit value="确定"/>
					</td>
					...
					<table width="770px">
						...
						<td style="width:400px;font-size:14px;">
							<s:if test="currenPage>1">
								<a href="kaoQinSearch.action?currenPage=<s:property value="currenPage-1"/>">上一页</a>
							</s:if>
						</td>
						<td style="width:400px;font-size:14px;">
							<s:if test="currenPage<maxPage">
							<a href="kaoQinSearch.action?currenPage=<s:property value="currenPage+1"/>">下一页</a>
							</s:if>
						</td>
