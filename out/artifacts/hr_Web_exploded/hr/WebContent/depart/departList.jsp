...
<div class="table-responsive"  style="margin-top: 20px;">
			<table class="table_list  table-condensed table-hover table-bordered  table-striped">
				...
				<tbody>
				  <c:forEach var="depart" items="${pageInfo.pageInfos }">
					<tr>
						<td>${depart.id }</td>
						<td>${depart.name }</td>
						<td><a href="searchEmpByCondition.do?deptName=${depart.name}">${depart.empCount}</a></td>
						<td>${depart.describe}</td>
						<td><button class="btn btn-primary btn-sm">删除</button>　
							<a href="depart/editDepart.jsp?id=${depart.id}&name=${depart.name}&desc=${depart.describe}" class="btn btn-primary btn-sm">编辑</a></td>
					</tr>
				  </c:forEach>
				</tbody>
				<tfoot>
						...
						<td colspan="9" style="text-align: right;"> 
							<ul class="pagination">
								<c:if test="${pageInfo.currentPage-1>0}">
								    <li>
								      <a href="departList.do?currentPage=${pageInfo.currentPage-1 }&pageSize=7" aria-label="Previous">
								        <span aria-hidden="true">&laquo;</span>
								      </a>
								    </li>
							    </c:if>
							    <c:forEach begin="1" end="${pageInfo.pageCount }" step="1" var="page">
								    <li><a id="${page}" href="departList.do?currentPage=${page}&pageSize=7">${page }</a></li>
							    </c:forEach>
							     <c:if test="${pageInfo.currentPage+1 <= pageInfo.pageCount }">
								    <li>
								      <a href="departList.do?currentPage=${pageInfo.currentPage+1 }&pageSize=7" aria-label="Next">
							  ...
							  <script type="text/javascript">
							  	$("a[id="+${pageInfo.currentPage}+"]").css("background-color","#99CCCC");
							  </script>
      					</td>
					</tr>
				</tfoot>
